...
//    domain: ['#FF6666', '#66FF66', '#6666FF', '#777777']
  ...
  private updateGpuSeries(date: Date = null) {
    ...
    for (const entry of this.gpus) {
      if (entry.series.length > ServerNewComponent.MAX_ENTRIES) {
        entry.series.splice(0, entry.series.length - ServerNewComponent.MAX_ENTRIES);
      }
    }
  ...
  private updateNetworkSeries(date: Date = null) {
    this.rawNetwork.push([ServerNewComponent.orNow(date), [this.node.netInfo.transmitting, this.node.netInfo.receiving]]);
    if (this.rawNetwork.length > ServerNewComponent.MAX_ENTRIES) {
      this.rawNetwork.splice(0, this.rawNetwork.length - ServerNewComponent.MAX_ENTRIES);
    }
  }
  ...
  private updateDiskSeries(date: Date = null) {
    this.rawDisk.push([ServerNewComponent.orNow(date), [this.node.diskInfo.writing, this.node.diskInfo.reading]]);
    if (this.rawDisk.length > ServerNewComponent.MAX_ENTRIES) {
      this.rawDisk.splice(0, this.rawDisk.length - ServerNewComponent.MAX_ENTRIES);
    }
  }
