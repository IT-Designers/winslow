...
export class ServersComponent implements OnInit, OnDestroy {
  ...
  private initGpuSeries() {
    ...
    for (let i = 0; i < ServersComponent.MAX_ENTRIES; i++) {
      var date = new Date();
  ...
  private updateCpuStatus() {
    ...
    if(this.isLive) {
      ...
      if (this.cpus.length > ServersComponent.MAX_ENTRIES) {
        this.cpus.shift();
      }
    }
  ...
  private updateMemoryStatus() {
    ...
    if(this.isLive) {
      ...
      for (const entry of this.memory) {
        if (entry.series.length > ServersComponent.MAX_ENTRIES) {
          entry.series.splice(
            0,
            entry.series.length - ServersComponent.MAX_ENTRIES
          );
        }
      }
      this.memory = [this.memory[0], this.memory[1], this.memory[2]];
  ...
  private updateNetworkSeries(date: Date = null) {
    ...
    if (this.rawNetwork.length > ServersComponent.MAX_ENTRIES) {
      this.rawNetwork.splice(
        0,
        this.rawNetwork.length - ServersComponent.MAX_ENTRIES
      );
    }
  }
  ...
  private updateDiskSeries() {
    ...
    if (this.rawDisk.length > ServersComponent.MAX_ENTRIES) {
      this.rawDisk.shift();
    }
  }
  ...
  private updateGpuStatus() {
    if(this.isLive) {
      if (this.gpus[0].series.length > ServersComponent.MAX_ENTRIES) {
  ...
  scaleNetwork() {
    if (this.network.length > ServersComponent.MAX_ENTRIES) {
      this.network.shift();
    }
