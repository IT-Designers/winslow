...
if [ "$WINSLOW_CA_CERT_DIR" != "" ]; then 
  ...
  for f in $(find "$WINSLOW_CA_CERT_DIR" -type f); do
    echo "Importing $f"
#    keytool -delete -alias "$f" -keystore -cacerts -storepass changeit -noprompt || true
    keytool -import -trustcacerts -keystore -cacerts -storepass changeit -noprompt -alias "$f-$(date +%s )" -file "$f"
  done
fi
